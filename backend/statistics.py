...
def get_recent_global_message_stats() -> DataFrameGroupBy:
    ...
    # Create a DatetimeIndex relative to end, then reverse so that it is monotonically 
    # increasing (required by pd.cut). Assume UTC (handled by Django).
    date_idx = pd.DatetimeIndex([end-DateOffset(hours=e) for e in range(0, 25)][::-1])
    
    # No need to localize again, it's already timezone aware
