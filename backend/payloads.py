...
def get_and_save_build_log(build_dir: Path, user: Optional[User], task_id: Optional[str]) -> Log:
...
def get_payload_and_copy(build_dir: Path, agent_id: uuid.UUID, agent: Agent) -> Path:
    ...
    media_path = Path(Endpoint.payload_file.field.upload_to) / f"{str(agent)}-{agent_id}"
